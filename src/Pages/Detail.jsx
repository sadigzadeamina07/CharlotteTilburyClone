...
function getAccordionItems(text) {
  ...
  const getShadeStatus = (shade) => {
    ...
    if (shade.outOfStock) return "Out of Stock"
    return null
  }
  ...
  const gallery = rawGallery.filter((img) => img?.type !== "video" && !img?.placeholder)
  ...
  const accordionSections = [
    { key: "details", title: "PRODUCT DETAILS", items: getAccordionItems(product.aboutTheProduct) },
    { key: "magic", title: "WHAT MAKES IT MAGIC?", items: getAccordionItems(product.ingredientsAndBenefits) },
    { key: "apply", title: "HOW TO APPLY", items: getAccordionItems(product.howToApply) },
    {
      key: "shipping",
...
Free standard delivery on orders over {formatPrice(FREE_SHIPPING_THRESHOLD_GBP, selectedCountry)}
  ...
  const oosBanner = (
    <div className="flex items-center gap-3 border border-[#c97b2a] bg-[#fff8f1] px-4 py-3.5 mb-4">
      <span className="text-[#c97b2a] text-[18px] shrink-0">⊙</span>
  ...
  const discBanner = (
    <div className="flex items-center gap-3 border border-[#856d6d] bg-[#f9f8f6] px-4 py-3.5 mb-4">
      <span className="text-[#856d6d] text-[18px] shrink-0">⊙</span>
      <p className="text-[14px] font-helveticaN text-[#340c0c]">
    ...
    <section className="mt-6 mb-6">
      {isSizeMode ? (
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            >
              {shades.map((s) => (
                <option key={s.name} value={s.name} disabled={isUnavailable(s)}>
                  {s.name}
                  {s.outOfStock ? " — Out of Stock" : ""}
                  {s.discontinued ? " — Discontinued" : ""}
                  {s.price ? `  —  ${formatPrice(s.price, selectedCountry)}` : ""}
                </option>
              ))}
            </select>
            <ChevronDown
        ...
        <>
          {shadeGroups.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              <button onClick={() => setActiveGroup(null)} className={groupBtnClass(activeGroup === null)}>
                All
              </button>
              {shadeGroups.map((group) => (
                <button key={group} onClick={() => setActiveGroup(group)} className={groupBtnClass(activeGroup === group)}>
                  {group}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-6 md:grid-cols-7 gap-1 mb-5">
            {visibleShades.map((shade) => {
              const status = getShadeStatus(shade)
              return (
                <button
                  key={shade.name}
                  onClick={() => selectShade(shade)}
                  title={`${shade.name}${status ? ` - ${status}` : ""}`}
                  className={
                ...
                >
                  ...
                  />
                  {isUnavailable(shade) && (
                    <span className="absolute inset-0 flex items-center justify-center pointer-events-none" style={diagonalLine} />
                  )}
                </button>
              )
            })}
            ...
            <div className="flex items-center gap-4">
              <div className="relative w-8 h-8 shrink-0">
                <img
                  ...
                  className={`w-full h-full object-cover ${selectedShadeStatus ? "opacity-40" : ""}`}
                />
                {selectedShadeStatus && (
                  <span className="absolute inset-0 pointer-events-none" style={diagonalLine} />
                )}
              </div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        <button className="cursor-pointer flex items-center justify-center gap-2 border border-[#eae6e6] py-3.5 hover:border-[#340c0c] transition-colors">
          ...
          <span className="font-helveticaN uppercase text-[12px] font-bold text-[#340c0c]">HOW TO APPLY</span>
        </button>
        <button className="cursor-pointer flex items-center justify-center gap-2 border border-[#eae6e6] py-3.5 hover:border-[#340c0c] transition-colors">
          <Camera size={16} strokeWidth={1.5} className="text-[#340c0c]" />
          <span className="font-helveticaN uppercase text-[12px] font-bold text-[#340c0c]">TRY ON</span>
        </button>
      </div>
    </section>
  )
      ...
      <div className="relative w-full md:w-[420px] h-full bg-white flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 z-[1000000]">
        ...
        {shadeGroups.length > 0 && (
          <div className="flex flex-wrap gap-2 px-4 py-3 border-b border-[#eae6e6]">
            <button onClick={() => setActiveGroup(null)} className={groupBtnClass(activeGroup === null).replace("text-[12px]", "text-[11px]").replace("px-4", "px-3")}>
              All
            </button>
            {shadeGroups.map((group) => (
              <button key={group} onClick={() => setActiveGroup(group)} className={groupBtnClass(activeGroup === group).replace("text-[12px]", "text-[11px]").replace("px-4", "px-3")}>
                {group}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {visibleShades.map((shade) => {
            const status = getShadeStatus(shade)
            const unavailable = isUnavailable(shade)
            return (
              <button
                key={shade.name}
                onClick={() => { selectShade(shade); setDrawerOpen(false) }}
                className={`w-full flex items-center justify-between p-4 ${selectedShade?.name === shade.name ? "bg-[#f9f8f6]" : "hover:bg-[#f9f8f6]"}`}
              >
                <div className="flex items-center gap-5">
                  ...
                  <div className="flex flex-col">
                    <span className={`font-helveticaN text-[15px] ${unavailable ? "text-[#856d6d]" : "text-[#340c0c]"}`}>
                      {shade.name}
                    </span>
                    {status && (
                      <span className="text-[11px] font-helveticaN text-[#856d6d] uppercase mt-0.5">
      ...
      {accordionSections.map(({ key, title: sectionTitle, items }) => (
        ...
        >
          <div className="flex justify-between items-center">
            <h3 className="font-optima uppercase text-[17px] font-medium text-[#340c0c] group-hover:text-[#856d6d] transition-colors">
              {sectionTitle}
            </h3>
            {openAccordions[key]
          ...
          {openAccordions[key] && (
            <div className="mt-5 flex flex-col gap-4">
              {items.map((item, i) => (
                <p key={i} className="text-[14px] text-[#340c0c] font-helveticaN">
                  {item.text}
                </p>
              ))}
            </div>
          ...
          <article className="border-b border-[#eae6e6] py-5">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-optima uppercase text-[17px] font-medium text-[#340c0c] group-hover:text-[#856d6d] transition-colors">
                  WATCH THE TUTORIAL
                </span>
              </div>
              {videoOpen
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
            ...
            <aside className="w-full lg:w-[63%] lg:sticky lg:top-8 lg:self-start">
              ...
              <div className="lg:hidden mb-6">
                ...
                {subTitle && (
                  <p className="text-[#856d6d] uppercase font-helveticaN text-sm mt-1">{subTitle}</p>
                )}
                {priceBlock}
              </div>
            ...
            <article className="w-full lg:w-[37%] pb-12 lg:sticky lg:top-8 lg:self-start lg:max-h-[calc(100vh-4rem)] lg:overflow-y-auto no-scrollbar">
              <header className="hidden lg:block">
                <div className="flex justify-between gap-4">
                  <h1 className="text-[28px] font-optima uppercase text-[#340c0c]">{title}</h1>
                  <button
                    onClick={() => toggleWishlist(cartProduct)}
                ...
                {subTitle && (
                  <p className="text-[#856d6d] uppercase font-helveticaN text-[14px] mt-1">{subTitle}</p>
                )}
      ...
      <aside className="md:hidden fixed bottom-0 left-0 w-full bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.15)] z-[60] flex flex-col border-t border-[#eae6e6]">
        <div className="px-4 py-3 w-full flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            {subTitle && (
              <span className="text-[#856d6d] uppercase text-[12px] font-helveticaN">{subTitle}</span>
            )}
          </div>
