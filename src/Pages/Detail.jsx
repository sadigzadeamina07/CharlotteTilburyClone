...
import {
  TbDiamondsFilled, TbDroplet, TbClock, TbFlower,
} from "react-icons/tb";
...
function Detail() {
  ...
  useEffect(() => {
    ...
    setActiveImage(0);
    const firstShade =
      product?.shades?.[0] || product?.detailPageData?.shades?.[0] || null;
  ...
  // Shade status helpers
  const shadeIsOOS = (shade) =>
    !!shade?.outOfStock ||
    (shade?.name || "").toLowerCase().includes("out of stock");

  const shadeIsDisc = (shade) =>
    !!shade?.discontinued ||
    (shade?.name || "").toLowerCase().includes("discontinued");

  const shadeStatusLabel = (shade) => {
    if (!shade) return null;
    if (shadeIsDisc(shade)) return "Discontinued";
    if (shadeIsOOS(shade)) return "Out of Stock";
    return null;
  };

  const isOutOfStock = selectedShade ? shadeIsOOS(selectedShade) : product.outOfStock;
  const isDiscontinued = selectedShade ? shadeIsDisc(selectedShade) : product.discontinued;
  ...
  const activeMediaUrl =
    typeof activeItem === "string" ? activeItem : activeItem?.url || product?.images?.main || "";

  const prevImage = () =>
    setActiveImage((prev) => (prev === 0 ? safeGallery.length - 1 : prev - 1));

  const nextImage = () =>
    setActiveImage((prev) => (prev >= safeGallery.length - 1 ? 0 : prev + 1));
  ...
  const productDetailsItems = getAccordionItems(
    "PRODUCT DETAILS",
    product.productDetails || product.aboutTheProduct || "",
    "Sparkle",
    ["ABOUT THE PRODUCT"]
  );
  const magicItems = getAccordionItems(
    "WHAT MAKES IT MAGIC?",
    product.whatMakesItMagic || product.aboutTheProduct || "",
    "Heart",
    ["WHAT MAKES IT MAGIC", "ABOUT THE PRODUCT"]
  );
  const ingredientItems = getAccordionItems(
    "INGREDIENTS",
    product.ingredients || product.ingredientsAndBenefits || "",
    "Leaf",
    ["INGREDIENTS + BENEFITS", "INGREDIENTS & BENEFITS"]
  );
  const applyItems = getAccordionItems("HOW TO APPLY", product.howToApply || "", "Drop");
  const shippingItems = getAccordionItems(
  ...
  const descSentences = rawDesc.split(". ");
  const shortDesc = descSentences.slice(0, 2).join(". ");

  // ─── Reusable inline pieces ────────────────────────────────────────────────

  // Out of stock / discontinued banners
  const oosBanner = (
  ...
  const shadePicker = shades.length > 0 && (
    <section className="mt-6 mb-6">
      ...
      <div className="grid grid-cols-6 md:grid-cols-7 gap-1 mb-5">
        {shades.map((shade) => (
          <button
            ...
            title={`${shade.name}${shadeStatusLabel(shade) ? ` - ${shadeStatusLabel(shade)}` : ""}`}
            className={
          ...
          >
            <img
              ...
              alt={shade.name}
              className={`w-full h-full object-cover ${shadeIsOOS(shade) || shadeIsDisc(shade) ? "opacity-40" : ""}`}
            />
            {(shadeIsOOS(shade) || shadeIsDisc(shade)) && (
              <span
      ...
      {/* Selected shade row + open drawer */}
      <button
        ...
        <div className="flex items-center gap-4">
          <div className="relative w-8 h-8 flex-shrink-0">
            <img
              ...
              className={`w-full h-full object-cover ${shadeStatusLabel(selectedShade) ? "opacity-40" : ""}`}
            />
            {shadeStatusLabel(selectedShade) && (
              <span
      ...
      <div className="relative w-full md:w-[420px] h-full bg-white flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 z-[1000000]">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {shades.map((shade) => (
            <button
              key={shade.name}
              onClick={() => { handleShadeSelect(shade); setDrawerOpen(false); }}
              className={
                selectedShade?.name === shade.name
            ...
            >
              <div className="flex items-center gap-5">
                ...
                <div className="flex flex-col items-start">
                  <span className={`font-helveticaN text-[15px] ${shadeIsOOS(shade) || shadeIsDisc(shade) ? "text-[#856d6d]" : "text-[#340c0c]"}`}>
                    {shade.name}
                  </span>
                  {shadeStatusLabel(shade) && (
                    <span className="text-[11px] font-helveticaN text-[#856d6d] uppercase tracking-wide mt-0.5">
                      {shadeStatusLabel(shade)}
                    </span>
                  )}
  ...
  const accordionList = (
    <div className="flex flex-col mb-8 border-t border-[#eae6e6]">
      {allAccordionSections.map(({ key, title: sectionTitle, items }) => (
        <article
          key={key}
        ...
        <section className="py-4 md:py-6">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
            ...
            <aside className="w-full lg:w-[63%] lg:sticky lg:top-8 lg:self-start">
              {/* Mobile: title + price above gallery */}
              ...
              {/* Mobile shade picker + banners */}
              <div className="lg:hidden mt-2">
                {shadePicker}
                {isOutOfStock && oosBanner}
                {isDiscontinued && discBanner}
              </div>
            </aside>
      ...
      <aside className="hidden md:flex fixed bottom-0 left-0 w-full bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.08)] z-[60] items-stretch h-[80px]">
        <div className="flex-1 max-w-[1470px] mx-auto w-full flex justify-between items-center px-8">
          ...
          <div className="flex items-center gap-8">
            ...
            <button
              onClick={isOutOfStock || isDiscontinued ? notifyMe : () => handleAddtoBasket(cartProduct)}
              className={
